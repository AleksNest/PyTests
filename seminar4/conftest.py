...
def make_folders():
    return ssh_checkout(f"{data.get('ip')}", f"{data.get('user')}", f"{data.get('password')}",
                        f"mkdir -p {data.get('folder_in')} {data.get('folder_out')} "
...
def make_files():
    ...
    for i in range(data.get('count')):
        ...
        if ssh_checkout(f"{data.get('ip')}",
                        f"{data.get('user')}",
                        f"{data.get('password')}",
                        f"cd {data.get('folder_ext')}; dd if=/dev/urandom of={filename} b"
                        f"s={data.get('bs')} count=1 iflag=fullblock",
                        ''):
...
@pytest.fixture()
def clear_folders():
    return ssh_checkout(f"{data.get('ip')}",
                        f"{data.get('user')}",
                        f"{data.get('password')}",
                        'rm -rf {}/* {}/* {}/* {}/*'.format(data.get('folder_in'), data.get('folder_out'),
                                                            data.get('folder_ext'), data.get('folder_ext2')),
                        "")


@pytest.fixture()
def make_bad_arx():
    ssh_checkout(f"{data.get('ip')}",
                 f"{data.get('user')}",
                 f"{data.get('password')}",
